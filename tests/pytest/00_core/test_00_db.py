...
def test_truncate_hello_world(db):
    ...
    row = db(DatabaseUserContext.APP).get_one("hello_count")
    assert row[0] == 0
...
def test_hello_select_value(db):
    values = {"bv1": "abc"}
    row = db(DatabaseUserContext.APP).get_one("hello_select_value", values)
...
def test_hello_schema_count(db):
    row = db(DatabaseUserContext.APP).get_one("hello_schema_count")
    assert row[0] == 4
